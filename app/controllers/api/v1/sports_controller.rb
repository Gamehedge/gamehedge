class Api::V1::SportsController < ApplicationApiController
  ...
  def index
    ...
    respond_to do |format|
      format.json { render json: @sports.to_json(:only => [:id, :name, :description, :te_uid, :url, :ggg], :methods => [:image_url, :image_url_medium, :image_url_thumb])}
      format.xml { render xml: @sports.to_json(:only => [:id, :name, :description, :te_uid, :url, :ggg], :methods => [:image_url, :image_url_medium, :image_url_thumb])}
    end
  end

  def show
    respond_to do |format|
      format.json { render json: @sport.to_json(:only => [:id, :name, :description, :te_uid, :url, :ggg], :methods => [:image_url, :image_url_medium, :image_url_thumb])}
      format.xml { render xml: @sport.to_json(:only => [:id, :name, :description, :te_uid, :url, :ggg], :methods => [:image_url, :image_url_medium, :image_url_thumb])}
    end
  end
