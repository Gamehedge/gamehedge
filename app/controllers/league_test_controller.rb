class LeagueTestController < ApplicationController
  def show
	...
	@next_events = TicketEvolutionService.new({:type => "events", :within => 25, :id => @league.te_uid, :geolocated => "true", :latitude => latitude, :longitude => longitude, :page => 1, :source => "league", :perpage => 50}).list
  end
end
